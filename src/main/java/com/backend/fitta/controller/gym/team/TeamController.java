...
public class TeamController {
    ...
    @GetMapping("/{teamId}")
    public ResponseEntity<BasicTeamInfo> findTeam(@PathVariable long teamId) {
        return ResponseEntity.ok(teamApiService.findById(teamId));
    }

    @Operation(summary = "전체 팀 조회 메서드", description = "전체 팀 정보를 조회 할 수 있습니다.")
    @GetMapping
    public ResponseEntity<Result<List<BasicTeamInfo>>> findAll() {
        return ResponseEntity.ok(teamApiService.findAll());
    }
